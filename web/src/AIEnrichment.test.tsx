...
describe('optional AI enrichment', () => {
  beforeEach(() => {
    vi.mocked(getCapabilities).mockReset().mockResolvedValue({ ai_enrichment_available: true, persistence_available: true });
  ...
  it('keeps AI unavailable when not configured', async () => {
    vi.mocked(getCapabilities).mockResolvedValue({ ai_enrichment_available: false, persistence_available: true });
    render(<App />);
